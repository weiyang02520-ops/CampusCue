...
# Register the campus SQLModel tables before astrbot's core is imported. The
# core creates every table it knows about when db.initialize() runs create_all
# (core_lifecycle.py:161), and the campus tables only land on the shared
# SQLModel metadata when this module is imported -- which otherwise happens
# during plugin reload at :246, too late. On a fresh checkout (no
# data/data_v4.db) that leaves campus_tasks, campus_sources, ... uncreated and
# every store call fails with "no such table" until some later import happens
# to pull campuscue.models in first. Importing here is idempotent and safe: the
# module has no side effects beyond registering tables.
import campuscue.models  # noqa: E402,F401
...
def check_env() -> None:
    if (sys.version_info.major, sys.version_info.minor) != (3, 12):
        logger.error(
            "CampusCue requires Python 3.12.x; found %s.%s.",
