...
def build_api_router() -> APIRouter:
    ...
    # CampusCue task board. Imported here rather than at module scope so a
    # missing campuscue package degrades to "no board" instead of breaking the
    # whole dashboard API.
    from campuscue.api.routes import router as campuscue_router
    from campuscue.api.setup import router as campuscue_setup_router

    router.include_router(campuscue_router)
    # Before the board router would matter if paths overlapped; they do not
    # (/campus/setup/* vs /campus/*), but the setup routes are registered second
    # so the board's existing surface stays the primary one.
    router.include_router(campuscue_setup_router)
    return router
