...
def initialize_runtime_bootstrap(log_obj: Any | None = None) -> bool:
    ok = configure_runtime_ca_bundle(log_obj=log_obj)

    # On Windows, the asyncio ProactorEventLoop logs "socket.send() raised
    # exception." via the "asyncio" logger every time a write lands on a
    # connection the client already closed -- which happens routinely when an
    # SSE client (the board) navigates away or loses Wi-Fi. With many clients
    # that single warning can flood the log file with hundreds of thousands of
    # lines. The event is informational (the transport is gone; the handler
    # cleans up), so quiet the logger to ERROR; real asyncio failures still
    # surface.
