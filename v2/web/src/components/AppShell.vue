...
const pageClass = computed(() => route.path === '/' ? 'route-home' : `route-${route.path.slice(1).replaceAll('/', '-')}`)
...
<template>
  <a class="skip-link" href="#main-content">跳到主要内容</a><div class="app-shell">
    <aside class="sidebar" aria-label="主导航"><div class="brand"><span class="brand-mark">C</span><span class="brand-copy"><strong>CampusCue</strong><small>cue your day</small></span></div><div class="workspace-label">我的校园工作台</div><nav><RouterLink v-for="item in items" :key="item.to" :to="item.to" :aria-current="route.path === item.to ? 'page' : undefined"><component :is="item.icon" :size="18" stroke-width="1.8" /><span>{{ item.label }}</span></RouterLink></nav><div class="sidebar-foot"><div class="sidebar-status"><div class="system-pill"><span class="status-dot" :class="{ offline: !app.online }"></span><span>{{ app.online ? '本地服务正常' : '正在重连服务' }}</span></div><button class="nav-link theme-toggle" @click="app.setTheme(app.resolvedTheme === 'light' ? 'dark' : 'light')"><component :is="app.resolvedTheme === 'light' ? Moon : Sun" :size="17" /><span>{{ app.resolvedTheme === 'light' ? '切换深色模式' : '切换浅色模式' }}</span></button></div></div></aside>
    <main id="main-content" :class="['main-column', pageClass]"><header class="topbar"><div><p class="eyebrow">{{ title }}</p><span class="topbar-area">{{ title }}</span><h1 class="sr-only">{{ title }}</h1></div><div class="topbar-actions"><div class="connection-indicator" :class="{ offline: !app.online }"><component :is="app.online ? Wifi : WifiOff" :size="15" />{{ app.online ? '已连接' : '离线' }}</div></div></header><slot /></main>
