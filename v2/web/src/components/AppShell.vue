...
<template>
  <a class="skip-link" href="#main-content">跳到主要内容</a><div class="app-shell">
    ...
    <main id="main-content" class="main-column"><header class="topbar"><div><p class="eyebrow">{{ title }}</p><span class="topbar-area">{{ title }}</span><h1 class="sr-only">{{ title }}</h1></div><div class="topbar-actions"><div class="connection-indicator" :class="{ offline: !app.online }"><component :is="app.online ? Wifi : WifiOff" :size="15" />{{ app.online ? '已连接' : '离线' }}</div></div></header><slot /></main>
