...
export const useResourcesStore = defineStore('resources', () => {
  const sources = ref<Source[]>([]); const providers = ref<Provider[]>([]); const messages = ref<Message[]>([]); const reminders = ref<Reminder[]>([]); const threads = ref<Array<{ conversation_id: string; source_id: number | null; message_count: number; last_activity: number | null }>>([]); const settings = ref<Settings>({ settings: { timezone:'Asia/Shanghai', theme:'system', message_retention_days:30, reminder_default_enabled:true, reminder_min_lead_seconds:60, reminder_quiet_start_hour:23, reminder_quiet_end_hour:8 }, restart_required: [] }); const loading = ref(false)
  ...
  async function loadThreads() { const result = await api.threads(); threads.value = Array.isArray(result) ? result : [] }
  async function loadSettings() { settings.value = await api.settings() }
  return { sources, providers, messages, reminders, threads, settings, loading, loadAll, loadSources, loadProviders, loadMessages, loadReminders, loadThreads, loadSettings }
})
