...
test.describe('M6.5.3 Dark UI Stage 2', () => {
  ...
  test('captures dialog and bottom sheet states with system-theme semantics', async ({ page }) => {
    ...
    await page.emulateMedia({ colorScheme:'light' }); await expect.poll(() => page.locator('html').getAttribute('data-theme')).toBe('light'); await page.reload(); await expect(page.locator('html')).toHaveAttribute('data-theme', 'light'); await expect(page.locator('.theme-picker button.active')).toHaveText('跟随系统')
