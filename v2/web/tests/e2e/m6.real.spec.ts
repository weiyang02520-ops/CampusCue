...
test.describe('M6.1 real M5 integration', () => {
  ...
  test('loads real API data and refreshes after a real SSE mutation', async ({ page }) => {
    await expect(page.getByText('高数第三章作业')).toBeVisible(); await page.goto('/tasks');
  ...
  test('runs task CRUD, real calendar, connections, provider test, settings, and data actions', async ({ page }) => {
    ...
    await page.getByRole('button', { name:'更多操作：Real create task' }).click(); await page.getByRole('menuitem', { name:'编辑' }).click(); await page.getByLabel('截止时间').fill(''); await page.getByRole('button', { name:'保存任务' }).click(); await page.getByRole('button', { name:'完成任务：Real create task' }).click(); await page.locator('.segmented').getByRole('button', { name:'已完成' }).click(); await expect(page.getByTestId(/task-/).filter({ hasText:'Real create task' })).toBeVisible();
    await page.goto('/calendar'); await expect(page.getByText('高数第三章作业')).toBeVisible(); await page.getByRole('button', { name:'下个月' }).click(); await page.getByRole('button', { name:'上个月' }).click();
    ...
    await page.goto('/providers'); await page.getByRole('button', { name:'添加提供商' }).click(); const providerDialog = page.getByRole('dialog', { name:'添加模型提供商' }); await providerDialog.getByLabel('名称').fill('Local fake provider'); await providerDialog.getByLabel('模型').fill('fake'); await providerDialog.getByLabel('连接地址').fill('http://127.0.0.1:6397/v1'); await providerDialog.getByRole('button', { name:'保存提供商' }).click(); await expect(page.getByText('Local fake provider')).toBeVisible(); await page.getByRole('button', { name:'测试' }).last().click(); await expect(page.getByText(/连接通过/)).toBeVisible();
