...
test.describe('M6.5.4.1 Theme UX semantic cleanup', () => {
  test('resolves System to Glass or Dark without mixed DOM states', async ({ page }) => {
    ...
    await expect(page.locator('html')).toHaveAttribute('data-visual-theme','glass'); await expect(page.locator('html')).toHaveAttribute('data-theme','light'); await expect(page.getByText('明暗模式')).toHaveCount(0); await expect(page.locator('.appearance-picker')).toHaveCount(0)
