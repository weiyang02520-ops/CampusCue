...
- **M3 = FINAL_FIX_COMPLETE AWAITING_EXTERNAL_REVIEW**（M3 + M3.1 + M3.2：quiet 窗外部不变量/overnight-only 契约、schema_meta 全局恰一行、composition-root 接线测试）
...
- **363 tests passed**（+28 M3 + 10 M3.1 + 9 M3.2）；package isolation PASS（fresh venv `.venv-m2iso` + apscheduler 3.11.3）；Anti-AstrBot PASS
