...
> 状态：**M7.0 PRODUCT CONTRACT = PASS；M7.1 PASS；M7.2 PASS；M7.3 PASS；M7 FINAL = PASS**
>
> 本文把 M7 Roadmap 收敛为可测试的产品契约。它不是代码实现授权。
>
> `M7 ROADMAP DESIGN = PASS`；`M7.0 = PASS`；`M7.1 = PASS`；`M7.2 = PASS`；`M7.3 = PASS`；`M7 FINAL = PASS`。
...
| M7-A07-CONTEXT-ISOLATION Agent thread binding | Source A history exists，客户端复用同一 conversation 切到 Source B | 直接调用 runtime/API | 在读取旧 history 或调用 Provider 前 fail closed；same-source continuity 保持；thread summary 仍绑定 A | provider request count/content、thread summary、message count | foreign-source history leakage > 0 = security blocker |
...
- API changes: **NONE**. Schema changes: **NONE**. M7.3 and M7 Final passed External Review；Real QQ M7 E2E remains `NOT_RUN` as an accepted limitation。
