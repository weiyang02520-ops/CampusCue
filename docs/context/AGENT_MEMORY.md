...
| 当前门 | **M0-M2 全部 FINAL PASS（含 M2b.2 REAL ENV）；M3 = FINAL_FIX_COMPLETE（M3.1+M3.2）AWAITING_EXTERNAL_REVIEW；M3 FINAL = NOT YET DECLARED；M4+ = NOT_AUTHORIZED** |
...
- **当前状态**：M0/M1/M2a/M2b.1/M2b.2 全部 PASS（M2 FINAL PASS @ 23083cb）。**M3 = FINAL_FIX_COMPLETE（M3.1 + M3.2：M3.2 = quiet 窗外部不变量 + schema_meta 全局单行 + composition-root 接线测试）AWAITING_EXTERNAL_REVIEW**。
...
- **当前状态**：**M0-M2 全部 FINAL PASS；M3 = FINAL_FIX_COMPLETE AWAITING_EXTERNAL_REVIEW**（M3 + M3.1 + M3.2：quiet 窗外部不变量（canonical is_inside_quiet_hours，overnight-only 契约）/schema_meta 全局恰一行（_precheck 版本分支前）/composition-root 真实接线测试（spy 生产构造器）；363 tests 全绿 + fresh venv + Anti-AstrBot）。**M3 FINAL = NOT YET DECLARED；M4+ NOT_AUTHORIZED**。
